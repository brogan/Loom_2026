error id: file://<WORKSPACE>/src/main/scala/org/loom/mysketch/MySketch.scala:drawBackgroundOnce.
file://<WORKSPACE>/src/main/scala/org/loom/mysketch/MySketch.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -java/awt/geom/drawBackgroundOnce.
	 -java/awt/geom/drawBackgroundOnce#
	 -java/awt/geom/drawBackgroundOnce().
	 -javax/sound/sampled/drawBackgroundOnce.
	 -javax/sound/sampled/drawBackgroundOnce#
	 -javax/sound/sampled/drawBackgroundOnce().
	 -org/loom/geometry/drawBackgroundOnce.
	 -org/loom/geometry/drawBackgroundOnce#
	 -org/loom/geometry/drawBackgroundOnce().
	 -org/loom/interaction/drawBackgroundOnce.
	 -org/loom/interaction/drawBackgroundOnce#
	 -org/loom/interaction/drawBackgroundOnce().
	 -org/loom/media/drawBackgroundOnce.
	 -org/loom/media/drawBackgroundOnce#
	 -org/loom/media/drawBackgroundOnce().
	 -org/loom/scaffold/drawBackgroundOnce.
	 -org/loom/scaffold/drawBackgroundOnce#
	 -org/loom/scaffold/drawBackgroundOnce().
	 -org/loom/scene/drawBackgroundOnce.
	 -org/loom/scene/drawBackgroundOnce#
	 -org/loom/scene/drawBackgroundOnce().
	 -org/loom/utility/drawBackgroundOnce.
	 -org/loom/utility/drawBackgroundOnce#
	 -org/loom/utility/drawBackgroundOnce().
	 -org/loom/transform/drawBackgroundOnce.
	 -org/loom/transform/drawBackgroundOnce#
	 -org/loom/transform/drawBackgroundOnce().
	 -drawBackgroundOnce.
	 -drawBackgroundOnce#
	 -drawBackgroundOnce().
	 -scala/Predef.drawBackgroundOnce.
	 -scala/Predef.drawBackgroundOnce#
	 -scala/Predef.drawBackgroundOnce().
offset: 22680
uri: file://<WORKSPACE>/src/main/scala/org/loom/mysketch/MySketch.scala
text:
```scala
/** *************************************************
 * MySketch
...
class MySketch(width: Int, height: Int) extends Sketch(width, height) {
  ...
  val scaleStrokeWidth: Boolean = if (globalConfig != null) globalConfig.scaleStrokeWidth else false
  private val quality: Int = Config.qualityMultiple //for adjusting sketch to different quality settings
  private var defaultLineWidth: Float = 1
  private var defaultPointSize: Float = 2f * quality
   ...
   */
  def makeRendererSetLibrary(n: String, xmlPath: Option[String] = None): RendererSetLibrary = {
    ...
    val min: Float = .1f * quality
    val max: Float = 5f * quality
    val inc: Float = .1f * quality
    val pauseMax = 17
  ...
  private def createShapeFromDef(shapeDef: ShapeDef): Shape2D = {
    ...
    val polygons: List[Polygon2D] = shapeDef.sourceType match {
      case ShapeConfigLoader.SOURCE_POLYGON_SET =>
        ...
        if (polySet != null) polySet.polySet
        else {
  ...
  def make2DSpriteList(): List[Sprite2D] = {
    ...
    // Get renderer set - try legacy name first, then fallbacks
    val rendererSet: RendererSet = {
      val legacySet = renderSetLibrary.getRendererSet("BlueOrangeGreenFilled")
      if (legacySet != null) legacySet
      else {
        val defaultSet = renderSetLibrary.getRendererSet("DefaultSet")
        if (defaultSet != null) defaultSet
        else renderSetLibrary.getRendererSet(0) // Get first available
      }
    }
  ...
  private def createSpriteFromDef(spriteDef: SpriteDef, shapeIndex: Int): Sprite2D = {
    ...
    // Create animator with the base factors
    val animator = Animator2D(
      spriteDef.animationEnabled,
      spriteParams.scaleFactor2D,
      spriteParams.rotFactor2D,
      spriteParams.speedFactor2D
    )
    ...
    if (spriteDef.scaleRangeX != (0.0, 0.0) || spriteDef.scaleRangeY != (0.0, 0.0) || hasScaleFactor) {
      ...
      animator.setRandomScale(scaleParams)
    }
    ...
    if (spriteDef.rotationRange != (0.0, 0.0) || hasRotationFactor) {
      ...
      animator.setRandomRotation(rotationParams)
    }
    ...
    if (spriteDef.translationRangeX != (0.0, 0.0) || spriteDef.translationRangeY != (0.0, 0.0) || hasSpeedFactor) {
      val translationParams: mutable.Map[String, Array[Double]] = mutable.Map(
        "x" -> Array(spriteDef.translationRangeX._1, spriteDef.translationRangeX._2),
        "y" -> Array(spriteDef.translationRangeY._1, spriteDef.translationRangeY._2)
      )
      animator.setRandomSpeed(translationParams)
    }
   ...
   * Takes the max totalDraws across all sprites.
   * Returns 0 (infinite) if any sprite has 0, or if no project config.
   */
  private def computeTotalDraws(): Int = {
    ...
    ProjectConfigManager.getSpriteConfig match {
      case Some(spriteLibrary) =>
        ...
        for (spriteSet <- spriteLibrary.spriteSets) {
          for (spriteDef <- spriteSet.sprites) {
            if (spriteDef.totalDraws == 0) return 0  // any infinite → all infinite
  ...
  override def setup(): Unit = {
    ...
    if (scaleStrokeWidth && quality > 1) {
      renderer.strokeWidth = renderer.strokeWidth * quality.toFloat
    }
  }
  ...
  def createPolygons(): List[Polygon2D] = {
    ...
    val pA: Polygon2D = PolygonCreator.makePolygon2D(3, .7, .7)
    val pB: Polygon2D = PolygonCreator.makePolygon2D(4, .7, .7)
