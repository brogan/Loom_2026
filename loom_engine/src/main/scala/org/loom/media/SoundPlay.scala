   ...
   def loadClip(): Unit = {
	 ...
	 //AudioInputStream stream = AudioSystem.getAudioInputStream(getClass().getResource(soundFile))
	     ...
	     clip.open(stream)
          ...
          case _: UnsupportedAudioFileException => println("No audio line available for: "+ soundFile)
          case _: LineUnavailableException => println("No audio line available for: "+ soundFile)
          case _: IOException => println("Could not read: "+ soundFile)
          case _: Exception => println("Problem with: "+ soundFile)
