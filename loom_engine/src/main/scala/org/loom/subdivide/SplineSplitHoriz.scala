...
/**
 * SPLIT_HORIZ subdivision for spline (bezier) polygons.
 * Geometry-aware: finds the most vertical pair of opposite sides and cuts through them,
 * producing a horizontal split line regardless of the polygon's internal side numbering.
 * Produces 2 output polygons.
 */
class SplineSplitHoriz(subdivObj: Subdivision, @unused middle: Vector2D, subP: SubdivisionParams, @unused polyType: Int) {
	...
	/**
	 * Find the rotation that produces the most horizontal split line.
	 * Even N: find the most vertical pair of opposite sides — cutting them produces a horizontal split.
	 * Odd N: find the vertex-to-opposite-side-midpoint line that is most horizontal.
	 */
	private def findBestRotation(): Int = {
		...
		if (N % 2 == 0) {
			...
			for (i <- 0 until half) {
				...
				val verticalness = (dy_i + dy_h) / (dx_i + dx_h + 0.001)
			...
			for (k <- 0 until N) {
				...
				val horizontalness = dx / (dy + 0.001)
