...
/**
 * TRI_BORD_C subdivision for spline (bezier) polygons.
 * For each original side, creates 3 triangles between the outer edge (split at a midpoint)
 * and the corresponding inset edge, producing 3N output polygons.
 *
 * For side i, the outer bezier is split at parameter t (from lineRatios) giving a midpoint M.
 * Triangle layout:
 *   Tri 1 (left):   outer[i] → M → scaled[i]
 *   Tri 2 (center): M → scaled[i+1] → scaled[i]   (inner side is reversed inset bezier)
 *   Tri 3 (right):  M → outer[i+1] → scaled[i+1]
 */
class SplineTriBordC(subdivObj: Subdivision, @unused middle: Vector2D, subP: SubdivisionParams, @unused polyType: Int) {

	val totNewPolys: Int = subdivObj.sidesTotal * 3
	...
	/**
	 * Split a bezier curve at parameter t using de Casteljau subdivision.
	 * Returns (leftHalf, rightHalf) where each is a 4-point bezier.
	 */
	private def splitBezierAt(side: Array[Vector2D], t: Double): (Array[Vector2D], Array[Vector2D]) = {
		...
		val M  = Formulas.lerp(M4, M5, t) // point on curve at t
		(Array(P0, M1, M4, M), Array(M.clone(), M5, M3, P3))
	}

	/**
	 * Create a bezier connector between two points using controlPointRatios.
	 */
	...
	private def makePolys(outerSides: Array[Array[Vector2D]], innerSides: Array[Array[Vector2D]]): Array[Polygon2D] = {
		...
		for (i <- 0 until subdivObj.sidesTotal) {
			// Determine split parameter based on lineRatios and continuous mode
			val t: Double = if (subP.continuous && (i % 2 != 0)) subP.lineRatios.y else subP.lineRatios.x

			// Split outer bezier at t
			val (leftHalf, rightHalf) = splitBezierAt(outerSides(i), t)
			val midpoint: Vector2D = leftHalf(3) // the split point on the curve

			// Anchor points on the inset polygon
			val scaledI: Vector2D = innerSides(i)(0)       // start of inset side i
			val scaledI1: Vector2D = innerSides(i)(3)      // end of inset side i (= start of next)

			// Tri 1 (left): outer[i] → midpoint → scaled[i]
			val tri1 = new Array[Vector2D](12)
			// Side 1: left half of split outer bezier
			tri1(0) = leftHalf(0).clone(); tri1(1) = leftHalf(1).clone()
			tri1(2) = leftHalf(2).clone(); tri1(3) = leftHalf(3).clone()
			// Side 2: midpoint → scaled[i]
			val conn1 = makeConnector(midpoint, scaledI)
			tri1(4) = conn1(0); tri1(5) = conn1(1); tri1(6) = conn1(2); tri1(7) = conn1(3)
			// Side 3: scaled[i] → outer[i]
