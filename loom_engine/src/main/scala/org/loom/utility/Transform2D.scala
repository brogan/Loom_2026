...
class Transform2D(var translation: Vector2D, var scale: Vector2D, var rotation: Vector2D) {
	
   override def toString(): String = {
      var s: String = ""
      s += "translation: " + translation.toString + ",  "
      s += "scale: " + scale.toString + ",  "
...
object Transform2D {
   ...
   */
   def scale(origPos: Vector2D, scale: Vector2D): Vector2D = {
	  //println("Transform2D scalex: " + scale.x + "   scale y: " + scale.y)
