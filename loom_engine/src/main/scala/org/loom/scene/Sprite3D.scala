...
class Sprite3D(var shape: Shape3D, var location: Vector3D, var size: Vector3D, val startRotation: Vector3D, val rotOffset: Vector3D, var animator: Animator3D, var rendererSet: RendererSet)  extends Drawable {
   ...
   */
   def draw(g2D: Graphics2D): Unit = {
      rendererSet.getRenderer(rendererSet.selectedIndex).mode match {
         case Renderer.POINTS => drawPoints(g2D, Camera.viewDistance, Camera.view)
         case Renderer.STROKED => drawLines(g2D, Camera.viewDistance, Camera.view)
         ...
         /**
   Draw, draws a particular Polygon2D in a Sprite
   UNTESTED ON SPRITE 3D
   Handles all sprite draw calls and passes them to specialised rendering methods depending on the rendering mode.
   @param g2D the Graphics2D context
   */
   ...
   */
   override def clone(): Sprite3D = {
      val s: Shape3D = shape.clone()
      val loc: Vector3D = location.clone()
      val sz: Vector3D = size.clone()
      val sr: Vector3D = startRotation.clone()
      val ro: Vector3D = rotOffset.clone()
      ...
      val a: Animator3D = animator.clone()
   ...
   */
   ...
   def drawPoints(g2D: Graphics2D, viewDistance: Double, view: View): Unit = {
      g2D.setColor(rendererSet.getRenderer(rendererSet.selectedIndex).strokeColor)
      g2D.setStroke(new BasicStroke(rendererSet.getRenderer(rendererSet.selectedIndex).strokeWidth))
         ...
         if (poly.getCenter().z + location.z > -(viewDistance)) {
            ...
            for (point <- poly.points) {
               if (!closerThanNearClipDistance(poly, viewDistance)) {
                  ...
                  g2D.draw(new Ellipse2D.Double(poly2D.points(count).x.toInt, poly2D.points(count).y.toInt, rendererSet.getRenderer(rendererSet.selectedIndex).pointSize, rendererSet.getRenderer(rendererSet.selectedIndex).pointSize))
                  count += 1
               }
            }
         } else {
   ...
   def drawFilled(g2D: Graphics2D, viewDistance: Double, view: View): Unit = {
      for (poly <- shape.polys) {
         if (!closerThanNearClipDistance(poly, viewDistance)) {
            ...
            val drawPoly: Polygon = PolygonCreator.convertPolygon2DToPolygon(poly2D)
            g2D.setColor(rendererSet.getRenderer(rendererSet.selectedIndex).fillColor)
            if (poly.polyType == PolygonType.LINE_POLYGON) {
               g2D.fill(drawPoly)
            } else {
               //iterate through each of the spline segments in the spline Polygon3d
