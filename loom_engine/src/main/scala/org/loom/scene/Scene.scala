...
class Scene() {
   ...
   */
   ...
   /**
   Gets a Drawable to the scene at a zIndex.
   @param sprite the Drawable to add
   @param zIndex the zIndex
   */
   def getSprite(zIndex: Int): Drawable = {
      sprites(zIndex)
   }
   /**
   ...
   */
   def draw(g2D: Graphics2D): Unit = {
      ...
      /**
   Draw, draws a specific Sprite in a Scene
   Calls draw on just that Sprite
   @param g2D the Graphics2D context
   */
   def drawSprite(g2D: Graphics2D, index: Int): Unit = {
      sprites(index).draw(g2D)
   }
         /**
   Draw, draws a specific Polygon2D in a Sprite in a Scene
   Calls draw on just that Polygon2D
   @param g2D the Graphics2D context
   */
   def drawSpritePoly(g2D: Graphics2D, spriteIndex: Int, polyIndex: Int): Unit = {
      sprites(spriteIndex).drawPoly(g2D, polyIndex)
   }
   /**
   Update
