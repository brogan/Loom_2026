    ...
    def setFullScreen(displayMode: DisplayMode, window: JFrame): Unit = {
        ...
        if (displayMode != null && device.isDisplayChangeSupported()) {
            try {
                ...
                case _: IllegalArgumentException => println( "IllegalArgumentException in set full screen (Simple Screen Manager)")
                case _: Exception => println( "Exception in set full screen (Simple Screen Manager)")
                case _ : Throwable => println( "Some other issue in set full screen (Simple Screen Manager)")
            }
        }
    }
