...
object PolygonCreator {
   ...
   */
   ...
   @return Polygon2D
   */
   def makePolygon2D(numberOfSides: Int, @unused width: Double, height: Double): Polygon2D = {
      ...
      for(i <- 1 until numberOfSides) {
    	 println(Transform2D.rotate(new Vector2D(0,-.5), 90))
