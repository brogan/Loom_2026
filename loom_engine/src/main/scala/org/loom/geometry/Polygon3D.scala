...
class Polygon3D(val points: List[Vector3D], val polyType: Int) {
   ...
   */
   def clone(newPoints: Array[Vector3D], vertexOrder: Array[Int]): Polygon3D = {
      val copyPoly: Array[Vector3D] = new Array[Vector3D](vertexOrder.size)
      
      // println ("Polygon3D, newPoints size: " + newPoints.size + "  vertexOrder size: " + vertexOrder.size + "  points size: " + points.size)
      // for (i <- 0 until points.size) {   println("i: " + i) }
